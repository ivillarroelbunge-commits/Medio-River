create or replace function public.player_stats_name_key(value text)
returns text
language sql
immutable
parallel safe
...
create or replace function public.player_stats_event_minute(value text)
returns integer
language sql
immutable
parallel safe
...
revoke all on function public.player_stats_name_key(text) from public;
revoke all on function public.player_stats_event_minute(text) from public;
grant execute on function public.player_stats_name_key(text) to anon, authenticated;
grant execute on function public.player_stats_event_minute(text) to anon, authenticated;

grant select on public.match_rating_players to anon, authenticated;
grant select on public.match_player_rating_stats to anon, authenticated;

drop policy if exists "Rating players are publicly readable" on public.match_rating_players;
create policy "Rating players are publicly readable"
on public.match_rating_players
for select
to anon, authenticated
using (true);

drop policy if exists "Rating aggregates are publicly readable" on public.match_player_rating_stats;
create policy "Rating aggregates are publicly readable"
on public.match_player_rating_stats
for select
to anon, authenticated
using (true);
...
crowd_ratings as (
  select player_id, competition_key, round(avg(match_average), 2) as rating
